...
| ID | Requirement | Method | Verified by | Status |
|---|---|---|---|---|
| REQ-PER-001 | The node shall process 2 MS/s across 160 channels in real time with a margin of at least 2x on the development machine. | T | `test_main_succeeds_within_a_generous_budget` | MET — measured 0.21 CPU-s/s, 4.8x |
| REQ-PER-002 | The channeliser shall cost no more than 0.5 CPU-seconds per signal second. | T | `test_pfb_benchmark_keeps_up_with_real_time` | MET — measured 0.177 |
...
| REQ-CAL-005 | Credible intervals shall contain the truth at the frequency they declare. | T | `test_the_ninety_five_percent_ring_contains_the_truth_that_often` | PARTIAL — verified against the model's own prior; the prior itself is unvalidated, which needs REQ-CAL-004 |
