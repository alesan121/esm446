...
| REQ-FUN-003 | The node shall detect emissions at a stated, configurable probability of false alarm, independent of the noise level. | T | `test_false_alarm_rate_holds_across_noise_levels`, `test_pure_receiver_noise_produces_no_false_alarms`, `test_tracking_the_level_cuts_the_rate_on_ambient_noise` | PARTIAL — design 1e-8. Met on synthetic noise across nine orders of magnitude. On real receiver noise at the shipped default (LNA 32/VGA 20): zero false alarms over a two-hour indoor run (2.88e10 cells), the two reports both attributing to one confirmed real emitter — see `docs/04_link_budget.md`. At high gain (VGA 40) outdoors the rate is 8e-6, because the environment is neither Gaussian nor stationary; absence of a crossing in one finite run bounds the rate rather than proving it |
...
| REQ-CAL-006 | Frequencies reported shall be consistent across captures to within 100 Hz. | T | `test_a_drifting_handset_is_still_one_emitter`, `test_the_measurement_has_unity_gain`, `test_spectral_tilt_does_not_move_the_answer`, `test_a_receiver_tuned_onto_the_carrier_is_refused` | PARTIAL — consistency verified. Absolute accuracy **not established**: every method tried (LTE notch, DVB-T band-edge) and an independent third-party tool (kalibrate, against real GSM base stations) disagree with each other by more than each one's own stated precision, up to 30x between this project's methods and kalibrate. That disagreement pattern is itself evidence for an unreliable clock source, not for a small crystal error — see [#60](https://github.com/alesan121/esm446/issues/60) and §9 of the V&V report |
