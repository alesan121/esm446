...
| REQ-CAL-006 | Frequencies reported shall be consistent across captures to within 100 Hz. | T | `test_a_drifting_handset_is_still_one_emitter`, `test_the_measurement_has_unity_gain`, `test_spectral_tilt_does_not_move_the_answer`, `test_a_receiver_tuned_onto_the_carrier_is_refused` | PARTIAL — consistency verified. Absolute accuracy **bounded, not calibrated**: measured against four cellular carriers spanning 806–1835 MHz, every result falls between -0.17 and -0.62 ppm, so the error is negative and under 1 ppm, which is under 450 Hz at 446 MHz. The references disagree far beyond their own scatter, so no tighter figure is claimed. A calibration needs a disciplined oscillator; see §9 of the V&V report |
